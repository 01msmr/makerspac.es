  ...
  class LanguageSwitcher {
    ...
    getFallbackTranslation(key) {
      const fallbacks = {
        ...
        clustering: 'Gruppierung (Clustering)',
    ...
    createSettingsPopover() {
      ...
      const languageIconsHTML = languages.map(lang => {
        // Beim Erstellen wird der aktuelle Zustand der Klasse aus der Modul-Variable gelesen
    ...
    `;
      ...
      languageHeader.querySelectorAll('[data-lang]').forEach(flag => {
        flag.addEventListener('click', (e) => {
          ...
          // Die visuelle Aktualisierung der Flaggen wird von updateSettingsLabels() übernommen
        });
      });
    ...
    `;
      ...
      <div class="settings-header-icons">
        <i class="fas fa-toggle-on settings-icon-btn" data-clustering="on" title="${this.t('clusteringOn')}"></i>
        <i class="fas fa-toggle-off settings-icon-btn" data-clustering="off" title="${this.t('clusteringOff')}"></i>
      </div>
    `;
      ...
      clusteringHeader.querySelectorAll('[data-clustering]').forEach(icon => {
        icon.addEventListener('click', (e) => {
          ...
          if (window.mapUtils && typeof window.mapUtils.toggleClustering === 'function') {
            ...
            // Manuelle Klassen-Setzung entfernt. Ruft nur die zentrale Update-Methode auf.
    ...
    `;
      ...
      this.updateClusteringToggleUI(); // Wichtig für den initialen Zustand
    }
    ...
    toggleSettingsPopover() {
      ...
      // Prüfe, ob es bereits sichtbar ist
      ...
      // ✨ WICHTIG: Aktualisiere vor dem Öffnen
      this.updateSettingsLabels();
      this.updateClusteringToggleUI(); // FIX: Aktualisiert den Schalter-Zustand vor dem Öffnen

      // Setze documentClickHandler
      setTimeout(() => {
    ...
    refreshUI() {
      ...
      // Wird nur aktualisiert, wenn das Popover sichtbar ist
      if (this.settingsPopover && !this.settingsPopover.classList.contains('is-hidden')) {
        this.updateSettingsLabels();
        this.updateClusteringToggleUI(); // FIX: Auch den Toggle-Zustand aktualisieren
      }
    }
    ...
    updateSettingsLabels() {
      ...
      // Die Indizes sind: [0] Sprache, [1] Farbschema, [2] Clustering, [3] Favoriten
      if (headers.length >= 4) {
      ...
      // ✨ FIX: Aktualisiere den Zustand der aktiven Flagge
      ...
      if (clusterOnBtn) clusterOnBtn.title = this.t('clusteringOn');
      if (clusterOffBtn) clusterOffBtn.title = this.t('clusteringOff');
    ...
    /**
     * Aktualisiert den visuellen Zustand des Clustering-Schalters basierend auf map.js.
     */
    updateClusteringToggleUI() {
      if (!this.settingsPopover) return;

      const clusteringIcons = this.settingsPopover.querySelectorAll('[data-clustering]');

      // Liest den Zustand direkt aus der Modul-Variable von map.js (dem WAHREN Zustand)
      const isClusteringActive = window.mapUtils && window.mapUtils.isClusteringEnabled();

      if (typeof isClusteringActive !== 'boolean') {
        return; // Abbruch, falls mapUtils noch nicht bereit (Race Condition)
      }

      clusteringIcons.forEach(icon => {
        const isToggleOn = icon.dataset.clustering === 'on';

        if (isClusteringActive === isToggleOn) {
          icon.classList.add('active');
        } else {
