  ...
  return `
            <div style="--status-color: ${statusColor};">
              <div class="popup-body-grid">
                ...
                ${(() => { const _t = (k) => window.i18n ? window.i18n.t(k) : ''; const _evLabel = _t('weekly.eventsCalendar'); const _hasWeekly = location.weekly && location.weekly.time && location.weekly.weekday <= 6; if (_hasWeekly) { const _isToday = location.weekly.weekday === new Date().getDay(); const _timeStr = String(location.weekly.time).padStart(4, '0').replace(/(\d{2})(\d{2})/, '$1:$2'); const _suf = _t('weekly.timeSuffix'); const _label = _isToday ? _t('weekly.today') : _t('weekdaysShort.' + location.weekly.weekday); const _calIcon = location.events ? `<a href="${location.events}" target="_blank" class="popup-events-link" aria-label="${_evLabel}" role="tooltip" data-microtip-position="bottom-right"><i class="fas fa-calendar-day"></i></a>` : `<i class="fas fa-calendar-day"></i>`; return `<div></div><div class="popup-weekly">${_calIcon} <span aria-label="${_t('weekly.tooltip')}" role="tooltip" data-microtip-position="bottom">${_label} — ${_timeStr}${_suf}</span></div>`; } if (location.events) { return `<div></div><div class="popup-weekly"><a href="${location.events}" target="_blank" class="popup-events-link"><i class="fas fa-calendar-day"></i> ${_evLabel}</a></div>`; } return ''; })()}
              </div>
